...
)
{
	...
	case ETCHASH_IO_FAIL:
		// ethash_io_prepare will do all ETCHASH_CRITICAL() logging in fail case
		goto fail_free_full;
	...
	case ETCHASH_IO_MEMO_SIZE_MISMATCH:
		...
		if (ethash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, true) != ETCHASH_IO_MEMO_MISMATCH) {
